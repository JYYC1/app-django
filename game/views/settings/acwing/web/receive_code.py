...
def receive_code(request):
    ...
    if players.exists():  # 如果该用户已存在，则无需重新获取信息，直接登录即可
        ...
        return redirect(reverse("index") + "?access=%s&refresh=%s % (str(refresh.access_token), str(refresh))") # 重定向到主页
    ...
    return redirect(reverse("index") + "?access=%s&refresh=%s" % (str(refrfesh.access_token), str(refresh)))
